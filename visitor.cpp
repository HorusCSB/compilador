...
antlrcpp::Any Visitor::visitDeclaracaoVariavel(gramaticaParser::DeclaracaoVariavelContext *ctx) {
    ...
    if (ctx->expressao()) {
        ...
        if (expressaoTexto.rfind("new ", 0) == 0) {
            ...
            ResultadoExpr resultado = std::any_cast<ResultadoExpr>(visit(ctx->expressao()));
...
antlrcpp::Any Visitor::visitExpressao(gramaticaParser::ExpressaoContext *ctx) {
    return std::any_cast<ResultadoExpr>(visit(ctx->expressaoSoma()));
}
...
antlrcpp::Any Visitor::visitExpressaoPrimaria(gramaticaParser::ExpressaoPrimariaContext *ctx) {
    ...
    if (ctx->acesso()) {
        return std::any_cast<ResultadoExpr>(visit(ctx->acesso()));
    }
...
antlrcpp::Any Visitor::visitExpressaoSoma(gramaticaParser::ExpressaoSomaContext *ctx) {
    ResultadoExpr resultado = std::any_cast<ResultadoExpr>(visit(ctx->expressaoProduto(0)));
    ...
    for (size_t i = 1; i < ctx->expressaoProduto().size(); ++i) {
        ...
        ResultadoExpr direito = std::any_cast<ResultadoExpr>(visit(ctx->expressaoProduto(i)));
...
antlrcpp::Any Visitor::visitExpressaoProduto(gramaticaParser::ExpressaoProdutoContext *ctx) {
    ResultadoExpr resultado = std::any_cast<ResultadoExpr>(visit(ctx->expressaoPrimaria(0)));

    for (size_t i = 1; i < ctx->expressaoPrimaria().size(); ++i) {
        std::string op = ctx->MULTIPLICACAO(i - 1) ? "*" : "/";
        ResultadoExpr direito = std::any_cast<ResultadoExpr>(visit(ctx->expressaoPrimaria(i)));
...
antlrcpp::Any Visitor::visitChamadaFuncao(gramaticaParser::ChamadaFuncaoContext *ctx) {
    ...
    if (ctx->argumentos()) {
        for (auto expr : ctx->argumentos()->expressao()) {
            argumentos.push_back(std::any_cast<ResultadoExpr>(visit(expr)));
        }
    }
    ...
    if (retorno.type() == typeid(ResultadoExpr)) {
        retornoFinal = std::any_cast<ResultadoExpr>(retorno);
    }
...
antlrcpp::Any Visitor::visitEstruturaIf(gramaticaParser::EstruturaIfContext *ctx) {
    ...
    ResultadoExpr condicaoIf = std::any_cast<ResultadoExpr>(visit(ctx->condicao()));
    ...
    for (auto elseifCtx : ctx->estruturaElsif()) {
        ResultadoExpr condElseIf = std::any_cast<ResultadoExpr>(visit(elseifCtx->condicao()));
...
antlrcpp::Any Visitor::visitEstruturaWhile(gramaticaParser::EstruturaWhileContext *ctx) {
    ...
    while (true) {
        ResultadoExpr cond = std::any_cast<ResultadoExpr>(visit(ctx->condicao()));
...
antlrcpp::Any Visitor::visitCondicao(gramaticaParser::CondicaoContext *ctx) {
    ResultadoExpr resultado = std::any_cast<ResultadoExpr>(visit(ctx->condicaoAnd(0)));

    for (size_t i = 1; i < ctx->condicaoAnd().size(); ++i) {
        ResultadoExpr dir = std::any_cast<ResultadoExpr>(visit(ctx->condicaoAnd(i)));
...
antlrcpp::Any Visitor::visitCondicaoAnd(gramaticaParser::CondicaoAndContext *ctx) {
    ResultadoExpr resultado = std::any_cast<ResultadoExpr>(visit(ctx->condicaoNot(0)));

    for (size_t i = 1; i < ctx->condicaoNot().size(); ++i) {
        ResultadoExpr dir = std::any_cast<ResultadoExpr>(visit(ctx->condicaoNot(i)));
...
antlrcpp::Any Visitor::visitCondicaoNot(gramaticaParser::CondicaoNotContext *ctx) {
    if (ctx->NOT()) {
        ResultadoExpr valor = std::any_cast<ResultadoExpr>(visit(ctx->condicaoNot()));
...
antlrcpp::Any Visitor::visitCondicaoPrimaria(gramaticaParser::CondicaoPrimariaContext *ctx) {
    if (ctx->expressao().size() == 2 && ctx->operadorComparacao()) {
        ResultadoExpr esq = std::any_cast<ResultadoExpr>(visit(ctx->expressao(0)));
        ResultadoExpr dir = std::any_cast<ResultadoExpr>(visit(ctx->expressao(1)));
    ...
    if (ctx->acesso()) {
        ResultadoExpr valor = std::any_cast<ResultadoExpr>(visit(ctx->acesso()));
